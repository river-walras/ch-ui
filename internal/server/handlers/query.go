...
// resultFilterMarker tags re-queries generated by the result grid's
// sort/filter UI; they are skipped in query history to avoid noise.
const resultFilterMarker = "ch-ui:result-filter"

const (
	maxHistoryQueryLen = 16 * 1024
	maxHistoryErrorLen = 4 * 1024
)

var (
	// CREATE/ALTER USER ... IDENTIFIED [WITH method] BY '<password>'
	redactIdentifiedRe = regexp.MustCompile(`(?i)(IDENTIFIED(?:\s+WITH\s+\w+)?\s+BY\s+)('(?:[^'\\]|\\.)*')`)
	// Table functions that take credentials as string arguments.
	credentialFnRe  = regexp.MustCompile(`(?i)\b(s3|s3Cluster|gcs|azureBlobStorage|deltaLake|iceberg|hudi|mysql|postgresql|mongodb|redis|odbc|jdbc|remote|remoteSecure)\s*\(`)
	stringLiteralRe = regexp.MustCompile(`'(?:[^'\\]|\\.)*'`)
)

// sanitizeQueryForHistory masks credentials before SQL is persisted at rest:
// IDENTIFIED BY passwords always, and every string literal when the query
// uses a table function that takes credentials as arguments.
func sanitizeQueryForHistory(q string) string {
	q = redactIdentifiedRe.ReplaceAllString(q, "$1'[REDACTED]'")
	if credentialFnRe.MatchString(q) {
		q = stringLiteralRe.ReplaceAllString(q, "'[REDACTED]'")
	}
	return q
}
...
func (h *QueryHandler) recordQueryHistory(connectionID, user, query, status, errMsg string, elapsedMS, rows int64) {
	...
	query = sanitizeQueryForHistory(query)
	if len(query) > maxHistoryQueryLen {
		query = truncateUTF8(query, maxHistoryQueryLen) + "\n/* … truncated */"
	}
	if len(errMsg) > maxHistoryErrorLen {
		errMsg = truncateUTF8(errMsg, maxHistoryErrorLen) + " … [truncated]"
	}
	go func() {
		if err := h.DB.CreateQueryHistoryEntry(database.CreateQueryHistoryParams{
			...
			slog.Warn("Failed to record query history", "error", err)
		}
	}()
}
...
func (h *QueryHandler) StreamQuery(w http.ResponseWriter, r *http.Request) {
	...
	recordHistory := func(status, errMsg string, rows int64) {
		if historyRecorded {
			return
		}
